...
**Always document what you attach.** Every artifact should have a companion
note explaining what it is, how it was generated, and why it matters. This
is how other people (and future agents) understand the experiment's outputs.

### The pattern: attach, then describe
...
sonde attach EXP-0001 output/*.nc                 # glob pattern

# 2. ALWAYS follow with a note describing what was attached
sonde note EXP-0001 "## Artifacts

- **figures/precip_delta.png** — Precipitation anomaly map from the CCN=1200
  run. Generated by `scripts/plot_precip.py` using output from WRF run 7.
  Shows 8% less enhancement in the outer rainband.

- **diagnostics/timeseries.csv** — 6-hourly CCN concentration time series
  at 3 probe locations. Extracted with `postprocess/extract_probes.jl`.

- **profiling_artifacts/** — GPU kernel profiling data from nsight-compute.
  `phase_summary.json` has the structured timing breakdown.
  `stdout.log` is the raw watchdog output."
```
