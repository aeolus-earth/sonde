...
**Always describe what you attach.** Every artifact should have a native
description explaining what it is, how it was generated, and why it matters.
Descriptions are shown as captions in the UI next to each file.

### The pattern: attach, then caption each file

```bash
# 1. Attach files — directories work too
...
# 2. For directories: list artifacts, then describe each one
sonde artifact list EXP-0001 --json
# Returns: ART-0001 phase_summary.json, ART-0002 stdout.log, etc.
...
sonde artifact update ART-0003 -d "Precip anomaly map, CCN=1200, from scripts/plot_precip.py"
```

### Single-file shortcut

For individual files, pass `-d` directly on attach:

```bash
sonde attach EXP-0001 figures/precip_delta.png \
  -d "Precipitation anomaly map from CCN=1200 run. Generated by scripts/plot_precip.py"
```

### Why native descriptions matter

- **Captions appear in the UI** next to each artifact in the file tree
- **Agents can read them** via `sonde artifact list` to understand what files are
- **Searchable** — artifact descriptions help find the right experiment
- **Better than notes** — descriptions are coupled to the file, not the experiment timeline

### Why this matters

- **Figures without context are useless.** A PNG of a graph tells you nothing
  if you don't know what axes are, what parameters produced it, or what code
  generated it.
- **Agents reading later need the provenance.** When the next agent runs
  `sonde show EXP-0001`, the notes + artifacts together should tell the
  complete story without needing to re-run anything.
- **The UI renders artifacts inline.** GIFs animate, CSVs become tables,
  JSON gets a collapsible tree, markdown gets formatted. But captions come
  from notes — so write them.

### Artifact descriptions (--description flag)

Individual artifacts also support inline descriptions:

```bash
sonde attach EXP-0001 figures/plot.png -d "Precip anomaly, CCN=1200"
```

Use `-d` for single-line captions. Use notes for multi-artifact documentation
...
# Note from file (for longer analysis)
sonde note EXP-0001 --file analysis-notes.md
```

Notes appear chronologically on the experiment detail page. Write them like
a lab notebook — what you did, what you saw, what it means.
